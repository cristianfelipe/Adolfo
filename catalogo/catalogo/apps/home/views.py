...
from catalogo.apps.home.forms import add_product_form, login_form
...
def juguetes_view(request, pagina):
	lista_prod = Juguete.objects.filter(status = True)
	...
	ctx  = {'juguetes':productos}
	return render_to_response('home/Juguete.html', ctx, context_instance =RequestContext(request))
...
def contacto_view(request):
	...
	if request.method == "POST":
		...
		if formulario.is_valid():
			...
			to_admin = 'akoztac@misena.edu.co'
...
def single_juguete_view(request, id_juguete):
	prod = Juguete.objects.get(id = id_juguete)
	ctx  = {'juguete':prod}
	return render_to_response('home/single_juguete.html',ctx,context_instance = RequestContext(request))
